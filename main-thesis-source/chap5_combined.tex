...
\label{chap:phase5-evaluation}

\section*{Introduction}
This chapter presents the final quantitative evaluation of the models developed during the preceding phases of the project. The transition from theoretical architecture to real-world deployment requires strict validation against predefined metrics. This chapter evaluates the accuracy of the detection models (Fire, Smoke, and Forklift) and their computational performance, specifically contrasting inference times on Neural Processing Units (NPUs) against traditional CPU and GPU hardware. Finally, it explores the performance of Zero-Shot Vision-Language Models deployed for shelf monitoring.

\section{Quantitative Evaluation of Supervised Models}
This section provides a purely quantitative evaluation of the supervised models developed during the preceding phases of the project. As the evaluation metrics (Precision, Recall, mAP) were formally defined in Chapter 4, this section focuses strictly on the empirical results.
...
\section{NPU Inference Latency Comparison}
While high mAP scores are desirable, the ultimate success of the EYE-D system depends on its real-time operational capacity on constrained hardware. This section evaluates the latency (inference time per frame in milliseconds) across different hardware profiles, acting as the centerpiece of this evaluation and justifying the thesis goal of ``Models Fit for NPUs.''
...
As detailed in Table~\ref{tab:inference-latency}, the NPU successfully executes both models well within the real-time threshold (typically <33ms for 30 FPS video). While the cloud GPU (Tesla T4) is faster, the Edge NPU provides the necessary localized processing without relying on network bandwidth or cloud compute costs. Crucially, the table demonstrates that by strictly selecting "Nano" architectures, the models successfully fit within the NPU's limited SRAM buffers. If larger models had been selected, the NPU would be forced to page memory to slower external RAM, causing the latency to spike exponentially and violate the real-time requirements.

\subsection{Comparison of YOLO Versions}
...
The quantitative evaluations in this chapter prove that it is entirely feasible to achieve high-precision industrial safety monitoring on resource-constrained Edge NPUs, provided the models are aggressively optimized and rigorously trained on domain-specific data. Every model successfully operated within the tight SRAM and thermal constraints of the target hardware, validating the end-to-end MLOps pipeline.
