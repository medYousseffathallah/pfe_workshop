...
The baseline capability of modern detection models is universally benchmarked against the COCO (Common Objects in Context) dataset, a large-scale collection of images spanning 80 diverse object categories. Performance on such datasets is quantified using Mean Average Precision (mAP), an evaluation metric that calculates the area under the Precision-Recall curve across multiple Intersection over Union (IoU) thresholds. In edge deployments, mAP serves as the primary counterbalance to Frames Per Second (FPS), dictating the acceptable trade-off between localization accuracy and real-time execution speed.

\subsection{Why YOLO is the Optimal Choice for Edge-AI and NPUs}
