$content = Get-Content "chap2_data.tex" -Raw -Encoding UTF8
...
$content = [regex]::Replace($content, '(?s)\\begin\{figure\}\[htbp\]\s*\\centering\s*\\includegraphics\[width=0\.85\\textwidth\]\{diagrams/crisp_dm\.png\}\s*\\caption\{CRISP-DM Cycle Diagram highlighting the iterative methodology and data loops within Phase 1\.\}\s*\\label\{fig:crisp-dm-phase1\}\s*\\end\{figure\}', $replacement)

Set-Content "chap2_data.tex" -Value $content -Encoding UTF8
